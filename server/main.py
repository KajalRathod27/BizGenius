...
# ─── Industry News (multi-API with guaranteed fallback) ───────────────────────
#
#  Priority order:
#   1. GNews          → GNEWS_API_KEY       (100 req/day, real-time)
#   2. NewsData.io    → NEWSDATA_API_KEY    (200 credits/day, ~12hr delay)
#   3. NewsAPI.org    → NEWS_API_KEY        (100 req/day, 24hr delay) ← your original
#   4. Mock articles  → always works, never crashes (presentation-safe)
#
#  Add whichever keys you have to your .env — the route tries each in order
#  and moves to the next one only if the current API fails or is quota-exhausted.
# ──────────────────────────────────────────────────────────────────────────────

def _normalize(articles: list, domain: str) -> list:
    """Shared normalizer — filters junk and returns clean dicts."""
    domain_lower = domain.lower()
    cleaned = []
    for a in articles:
        ...
        cleaned.append({
            ...
            "description": desc,
            "url":         a.get("url") or "#",
            "source":      a.get("source_name") or a.get("source", {}).get("name", "Unknown"),
            "published":   a.get("publishedAt") or a.get("pubDate") or "",
        })
    # prefer articles that actually mention the domain
    relevant = [a for a in cleaned if domain_lower in a["title"].lower() or domain_lower in a["description"].lower()]
    return (relevant[:8] if relevant else cleaned[:6])


@app.get("/news")
def news(domain: str = Query("SaaS")):

    # ── 1. GNews (real-time, easiest) ────────────────────────────────────────
    ...
    if gnews_key:
        try:
            ...
            if resp.status_code == 200:
                ...
                # GNews uses nested source object — reformat to match _normalize
                for a in raw:
                    a["source_name"] = a.get("source", {}).get("name", "Unknown")
                articles = _normalize(raw, domain)
                if articles:
                    print("✅ News served by: GNews")
                    return {"articles": articles, "source": "gnews"}
            else:
                print(f"GNews status {resp.status_code}: {resp.text[:120]}")
        except Exception as e:
            print("GNews error:", e)
    ...
    if newsdata_key:
        try:
            ...
            if resp.status_code == 200:
                ...
                # NewsData uses source_id instead of source object
                for a in raw:
                    a["source_name"] = a.get("source_id", "Unknown").replace("-", " ").title()
                    a["url"]         = a.get("link", "#")
                articles = _normalize(raw, domain)
                if articles:
                    print("✅ News served by: NewsData.io")
                    return {"articles": articles, "source": "newsdata"}
            else:
                print(f"NewsData status {resp.status_code}: {resp.text[:120]}")
        except Exception as e:
            print("NewsData error:", e)

    # ── 3. NewsAPI.org (original — kept as third option) ─────────────────────
    ...
    if newsapi_key:
        try:
            ...
            if resp.status_code == 200:
                ...
                articles = _normalize(raw, domain)
                if articles:
                    print("✅ News served by: NewsAPI.org")
                    return {"articles": articles, "source": "newsapi"}
            else:
                print(f"NewsAPI status {resp.status_code}: {resp.text[:120]}")
        except Exception as e:
            print("NewsAPI error:", e)

    # ── 4. Mock fallback — ALWAYS works, presentation-safe ───────────────────
    print("⚠ All news APIs exhausted — serving mock articles")
    today = datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")
    return {
        "articles": [
            {
                "title":       f"How {domain} Startups Are Disrupting the Market in 2025",
                "description": f"A deep dive into the latest trends shaping the {domain} ecosystem and what founders need to know.",
                "url":         "https://techcrunch.com",
                "source":      "TechCrunch",
                "published":   today,
            },
            {
                "title":       f"Top {domain} Companies Raise Record Funding This Quarter",
                "description": f"Investors are doubling down on {domain} as the sector shows strong growth and product-market fit signals.",
                "url":         "https://forbes.com",
                "source":      "Forbes",
                "published":   today,
            },
            {
                "title":       f"{domain} Sector Outlook: What Founders Need to Know",
                "description": "Expert analysis on risks, opportunities, talent wars, and market timing for early-stage teams.",
                "url":         "https://inc.com",
                "source":      "Inc. Magazine",
                "published":   today,
            },
            {
                "title":       f"5 Lessons From Successful {domain} Founders",
                "description": "We interviewed 20 founders across the space — here's what separated the winners from the rest.",
                "url":         "https://venturebeat.com",
                "source":      "VentureBeat",
                "published":   today,
            },
            {
                "title":       f"The Future of {domain}: AI, Automation & What's Next",
                "description": f"How emerging technologies are reshaping the {domain} landscape and creating new opportunities.",
                "url":         "https://wired.com",
                "source":      "Wired",
                "published":   today,
            },
